...
export default function App() {
  ...
  const displayedPredictions = useMemo(() => {
    return [...recentPredictions, ...seedPredictions(selectedModel)].slice(0, 8);
...
function seedPredictions(model: ModelInfo | null): PredictionRow[] {
  const key = model?.model_key ?? "pd_speech_features_local_VQC";
  return [
    { source: "sample", row_index: 0, probability: 0.842, predicted_label: "Parkinson's (PD)", model_key: key },
    { source: "sample", row_index: 1, probability: 0.136, predicted_label: "Healthy Control", model_key: key },
    { source: "sample", row_index: 2, probability: 0.673, predicted_label: "Parkinson's (PD)", model_key: key },
  ];
}
