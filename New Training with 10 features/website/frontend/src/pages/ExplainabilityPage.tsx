...
import type { FeatureImportance, GroupedExplanation, ModelInfo, PredictionRow } from "../types";
import type { PageId } from "../App";
import { ExplainabilityChatbot } from "./ExplainabilityChatbot";
...
export function ExplainabilityPage({
  ...
  return (
    <div className="page-stack explainability-page">
      <header className="page-heading">
        <div>
          <h1>Explainability</h1>
          <p>Visualize grouped SHAP contributions and ask local questions about the latest prediction.</p>
        </div>
      ...
      <section className="panel explainability-panel" id="explainability">
            ...
            <div className="explainability-chart-card">
              <div className="explainability-title">
                ...
                <div>
                  ...
                  <p>Grouped voice-signal drivers are shown as percentage-point movements from the model baseline.</p>
                </div>
              </div>

              <div className="grouped-chart" style={{ height: Math.max(groups.length * 58 + 28, 230) }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={groups} layout="vertical" margin={{ top: 8, right: 16, left: 8, bottom: 8 }}>
              ...
              <div className="color-meaning-box" aria-label="Chart color meaning">
                <span>
                  <i className="legend-swatch increase" />
                  Red means the grouped features raised the displayed PD probability.
                </span>
                <span>
                  <i className="legend-swatch decrease" />
                  Green means the grouped features lowered it from the model baseline.
                </span>
              </div>

              <FeatureGroupList groups={groups} />
            </div>
        ...
        <ExplainabilityChatbot prediction={prediction} selectedModel={selectedModel} />
      </section>
...
function FeatureGroupList({ groups }: { groups: GroupedExplanation[] }) {
  return (
    <div className="group-feature-list" aria-label="Features included in each SHAP group">
      <div className="group-feature-list-heading">
        <h3>Features in each group</h3>
        <span>{groups.length} groups</span>
      </div>
      <div className="group-feature-grid">
        {groups.map((group) => (
          <div className="group-feature-card" key={group.name}>
            <div className="group-feature-card-header">
              <strong>{group.name}</strong>
              <span>{formatContribution(group.value)}</span>
            </div>
            <div className="feature-chip-list">
              {(group.features?.length ? group.features : [`${group.featureCount} grouped features`]).map((feature) => (
