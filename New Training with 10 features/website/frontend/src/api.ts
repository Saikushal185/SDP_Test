...
export function fetchConfusionMatrix(modelKey: string): Promise<ConfusionMatrix> {
  return fetchJson<ConfusionMatrix>(`/api/confusion-matrix/${modelKey}`);
}
...
export async function runAudioPrediction(options: {
  modelKey: string;
  file: File;
}): Promise<{ predictions: PredictionRow[]; extractedFeatures: ExtractedFeatures }> {
  const form = new FormData();
  form.append("model_key", options.modelKey);
  form.append("file", options.file);
  ...
  return {
    ...
    extractedFeatures: payload.extracted_features,
  };
}
